...
void ParticleClassVector::AddParticle(int j, double p) {
	pcv[j].PushBackParticle(Particle(p));
}
...
void ParticleClassVector::UpdateBoundary() {
	...
	for (int i = 1; i < pcv.size(); i++) {
		//cout << "i: "<<i<<endl;
		pcv[i].SortMe();
		//cout<<"test pcv 1"<<endl;
		x3 = pcv[i].GetMinDiameter();
		//cout<<"test pcv 2"<<endl;
		x4 = pcv[i].GetMaxDiameter();
		//cout<<"test pcv 3"<<endl;
