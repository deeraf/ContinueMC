...
Calculator::Calculator(LogNormalDistribution logNormalDistribution, double simulationTime, double classTolerant, unsigned int maxParticleNumber, double interval, double wasserDampMasse,
		double glycerinDampMasse, double stickstoffDampMasse) {
	srand(time(NULL));
	ps = ParticleSystem(wasserDampMasse, glycerinDampMasse, stickstoffDampMasse); //REDUNDANT
	pcv = logNormalDistribution.Initialize("0_MCOutput.txt","0classSize.txt"); //first file number of particles second file diameter of classes third file number of classes
	totalSimulationTime = simulationTime;
	tolerant = classTolerant;
	maxNumberOfParticles = maxParticleNumber;
}
...
void Calculator::DoSimulation() {
	...
	ifstream volumenFile;
	volumenFile.open("00_C.txt");
	if(volumenFile.fail()){
		      cout << "error opening volume file" << endl;
		}
	volumenFile.seekg(16L, ios::beg);
	volumenFile >> volumen; 
	
	//volumen = CalVolumen(maxNumberOfParticles);
	cout<<"init vol: "<< volumen <<endl;
	iniVolumen = volumen;
	...
	while (currentTime <= totalSimulationTime) {
		...
		if (currentTime >= 1E-4) {
			if (currentTime >= currentFinePaintTime) {
				...
				logger.WriteToFileParticle(pcv, volumen, currentPictureIndex, ps.wasserDampfMasse, ps.glycerinDampfMasse, ps.stickStoffDampfMasse);
		...
		if (pcv.GetNumberOfParticles() >= maxNumberOfParticles) {
			cout<<"vol2: "<< volumen<<endl;
			cout<<"GetNumberOfParticles: " << pcv.GetNumberOfParticles() <<endl;
...
double Calculator::CalVolumen(unsigned int max) {
	...
	cout<<"diameter: "<<ps.CalKeimGroesse()<<endl;
	cout<<"beta: "<< beta<<endl;
	cout<<"C: "<< C <<endl;
	cout<<"Jo"<< Jo<<endl;
	cout<<"max: "<< max<<endl;
